...
class PiperRobotConfig(RobotConfig):
    ...
    # 主从录制模式下，leader_arms 与 follower_arms 的 key 需要一一对应。
    # 例如 follower_arms={"left": can1, "right": can0}，
    # 那么 leader_arms 也应使用同样的左右 key 约定。
    leader_arms: dict[str, MotorsBusConfig] = field(default_factory=lambda: {})
    follower_arms: dict[str, MotorsBusConfig] = field(
        default_factory=lambda: {
    ...
    # 硬件已经完成主从联动时，只配置 follower_arms，并开启 passive_recording。
    # 这样软件只采集从臂状态和相机，不再对机械臂做 leader->follower 控制。
